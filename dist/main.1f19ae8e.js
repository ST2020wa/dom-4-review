...
var div = dom.create('<div>test的弟弟</div>');
var div2 = dom.create('<div>test的哥哥</div>');
var div3 = dom.create('<div>test的孩子</div>');
var div4 = dom.create('<div>test的妈妈</div>');
dom.after(test, div);
dom.before(test, div2);
dom.append(test, div3);
dom.wrap(test, div4);
dom.remove(div3);
var nodes = dom.empty(window.emptyTest);
...
dom.find('.red', test2);
var findSib = dom.siblings(dom.find('#sis')[0]);
var preSib = dom.previous(dom.find('#sis')[0]);
var nextSib2 = dom.next(dom.find('#sis')[0]);
var t = dom.find('#travel')[0];
dom.each(dom.children(t), function (n) {
  return dom.style(n, 'color', 'blue');
});
var sis = dom.find('#sis')[0];
...
if ((!parent || !parent.isParcelRequire) && typeof WebSocket !== 'undefined') {
  ...
  var ws = new WebSocket(protocol + '://' + hostname + ':' + "53402" + '/');
